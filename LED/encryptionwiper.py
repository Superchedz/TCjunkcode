...
# custom code version by Graham Chedzoy to turn encryption off a basestation





# just run it and it'll turn everything off, if you want it back on, use the standard program
# which doesn't allow you to set a value...which is a bit shit
# really this code should have an argument to let you turn it off, but i've got better things
# to do
...
#            if self._defaultPANID == self._panID and self._encryption == False and self._encryptionKey == self._defaultEncryptionKey:
              self.logger.info("Running Grahams default settings reset code")

              self._generateOFFSetings()
              if self._applySettings():
                  if self._saveSettings():
                      self.logger.info ("Default setting have been successfully applied")
                      self._printSettings()
              else:
...
#                        self.exit()
    ...
    def _generateOFFSetings(self):
        self.logger.info("Generating default settings")
 # set things to default again
        self._panID = "5AA5"  
        self._encryptionKey = "000102030405060708090A0B0C0D0E0F"
        ...
        if self._at.enterATMode():
            ...
            if self._at.sendATWaitForOK("ATID{}".format(self._panID)):
                ...
                if not panID:
                    ...
                    return False
            if self._at.sendATWaitForOK("ATEE0"):
                ...
                if not encryption:
                    ...
                    print "Encryption is off now - ATEE0"
